...
class ReactAgent(
    ...
    def reset(self, goal):
        ...
        # refine instruction
        system_message += self.split["instruction"][0] + str(self.instruction) + self.split["instruction"][-1]
    ...
    def make_prompt(self,
        ...
        # history = self.memory[-self.memory_size+2:]
        # input_prompt = query + "\n".join([item["from"] + ": " + item["value"] for item in history])
        ...
        if len(self.memory) > 2:
            messages.extend( self.memory[2:][-self.memory_size+2:] )
        input_prompt = self.llm_model.make_tool_prompt(messages)
    ...
    def run(self, init_prompt_dict=None):
        ...
        input_prompt = self.make_prompt(need_goal=self.need_goal,
                                        ...
                                        system_message=self.memory[0]["value"])
        
        self.log_example_prompt(input_prompt)
        success, response = self.llm_model.generate(self.memory[0]["value"], input_prompt)
            # print(input_prompt)
